...
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	...
	if err := func() error {
		...
		klog.Warningf("FFFIIIIXXXX %s", len(items))
		klog.Warningf("FFFIIIIXXXX DeepSize(items) %s", DeepSize(items))
		klog.Warningf("FFFIIIIXXXX DeepSize(items[0]) %s", DeepSize(items[0]))
...
func (r *Reflector) syncWith(items []interface{}, resourceVersion string) error {
	return r.store.Replace(items, resourceVersion)
}
