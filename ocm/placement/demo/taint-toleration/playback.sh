...
#kubectl patch managedcluster ${C1} -p '{"spec":{"taints":[{"effect":"NoSelect","key":"gpu","value":"true"}]}}' --type=merge
#kubectl patch managedcluster ${C2} -p '{"spec":{"taints":[{"effect":"NoSelect","key":"gpu","value":"true"}]}}' --type=merge
...
clear 
p "User Case: User workload could be transferred to another available managed cluster after a tolerated time."
#p "Clear the taint"
pe "kubectl get managedclusters -A -o=jsonpath='{range .items[*]}{.metadata.name}{\"\\t\"}{.spec.taints}{\"\\n\"}{end}'"
